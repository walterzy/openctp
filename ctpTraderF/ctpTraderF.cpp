...
//		_semaphore.wait();
	...
	void CancelOrder()
	{
		std::string ExchangeID,InstrumentID;
		int OrderSysID;
		std::cout << "请输入: ExchangeID, InstrumentID, OrderSysID" << std::endl;
		std::cin >> ExchangeID >> InstrumentID >> OrderSysID;
		OrderCancel(ExchangeID.c_str(), InstrumentID.c_str(), OrderSysID);
	}

	int OrderCancel(const char* ExchangeID, const char* InstrumentID, int OrderSysID)
	{
